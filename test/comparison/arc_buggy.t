  ...
  [Error] File "./arc_buggy.rav", line 133, columns 12-27:
  133 |             fold arcInv(x);
                    ^^^^^^^^^^^^^^^
  Verification Error: Failed to fold predicate. The body of the predicate may not hold at this point.
  [Error] File "./arc_buggy.rav", line 25, columns 49-67:
  25 |         exists v: Int :: (v <= 0 ? noTokens(x) : tokenCounter(x, v)) && own(x.c, v, 1.0)
                                                        ^^^^^^^^^^^^^^^^^^
