...
Everything up to this test's own directory is stripped, and one record per line, so the
expected output does not depend on where the test happens to run -- the sandbox path
differs per run and per platform. The primary diagnostic keeps pointing at the file being
checked: an LSP diagnostic is displayed in the document it is reported against, so that
one has nowhere else to go.

  $ dune exec -- raven --lsp-mode -q ./lsp_related_loc.rav | sed 's|"file":"[^"]*/lsp/|"file":"|g; s|},{|}\n{|g'
