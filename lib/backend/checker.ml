      ...
      | Spec (spec_kind, spec) -> (
          ...
          | Assert -> (
              ...
              | false ->
                  ...
                  Error.smt_error stmt.stmt_loc
                    (Option.value
                       (Stmt.spec_error_msg spec curr_callable)
                       ~default:"Assertion is not valid")
      ...
      then Rewriter.return ()
        ...
        let cmd =
          SmtLibAST.mk_declare_fun ~loc:call_decl.call_decl_loc fully_qual_name
            ...
            (Type.mk_prod call_decl.call_decl_loc
                  ...
                  (Expr.mk_eq
                     (Expr.mk_app ~typ:(Expr.to_type ret_tuple)
                        ...
                        (List.map call_decl.call_decl_formals ~f:(fun arg ->
                             Expr.from_var_decl arg)))
                     ret_tuple
                  :: List.map call_decl.call_decl_precond ~f:(fun post ->
                         post.spec_form)))
               (Expr.mk_and
  ...
  | FuncDef { func_body = Some expr } -> (
      ...
      then Rewriter.return ()
        ...
        and spec_expr =
          Expr.mk_binder Forall call_decl.call_decl_formals
            ~trigs:
              [
                [
                  Expr.mk_app ~typ:Type.bot (Var fully_qual_name)
                    (List.map call_decl.call_decl_formals ~f:(fun arg ->
                         Expr.from_var_decl arg));
                ];
              ]
        ...
        | true -> (
            ...
            | _ -> assume_expr post_cond_expr)
        | false ->
            Error.smt_error call_decl.call_decl_loc
