    ...
    | Basic basic_stmt -> (
        ...
        | Assign assign ->
            ...
            let+ new_expr = f assign.assign_rhs in
            {
              stmt with
              stmt_desc =
                Basic (Assign { assign_lhs = new_lhs; assign_rhs = new_expr });
            }
        | Bind bind_desc ->
...
module Symbol = struct
  ...
  let reify_type_def loc (_name, symbol, subst) :
...
let is_local loc qual_ident s =
  ...
  Logs.debug (fun m ->
      m "Rewriter.is_local: qual_ident = %a" QualIdent.pr qual_ident);
...
module ProgUtils = struct
  ...
  let find_highest_valid_scope_qi loc (qi : qual_ident) : qual_ident t =
    let open Syntax in
    Logs.debug (fun m ->
        m "Rewriter.ProgUtils.find_highest_valid_scope_qi: qi = %a"
    ...
    Logs.debug (fun m ->
        m "Rewriter.ProgUtils.find_highest_valid_scope_qi: Found scope = %a"
  ...
  let intros_type_module ~(loc : location)
    ...
    Logs.debug (fun m ->
        m "Rewriter.ProgUtils.intros_type_module: symbol = %a" AstDef.Symbol.pr
          symbol);
  ...
  let rec does_symbol_implement_ra (symbol : AstDef.Module.symbol) : bool t =
    Logs.debug (fun m ->
        m "Rewriter.ProgUtils.does_symbol_implement_ra: symbol = %a"
          AstDef.Ident.pr
  ...
  let field_get_ra_qual_iden (field : AstDef.Module.field_def) =
    let field_type =
      ...
      | _ ->
          Error.error field.field_loc
            "Rewriter.ProgUtils.field_get_ra_module: Expected field definition"
    in
    ...
    | _ ->
        Error.error field.field_loc
          "Rewriter.ProgUtils.field_get_ra_module: Expected field type to be a \
           variable"
  ...
  let get_field_utils_id loc field_name : expr t =
    ...
    let field_type =
      ...
      | _ ->
          Error.error loc
            "Rewriter.ProgUtils.get_field_utils_id: Expected field definition"
    in
    ...
    let field_elem_type =
      ...
      | _ ->
          Error.error loc
            "Rewriter.ProgUtils.get_field_utils_id: Expected field type"
    in
  ...
  let pred_ra_constr_qual_ident loc pred_name =
    ...
    | AstDef.Module.CallDef c -> (
        ...
        | _ ->
            Error.error loc
              "Rewriter.ProgUtils.pred_ra_constr_qual_ident: Expected pred \
               definition")
    | _ ->
        Error.error loc
          "Rewriter.ProgUtils.pred_ra_constr_qual_ident: Expected pred \
  ...
  let pred_in_types pred_name =
    ...
    | _ ->
        Error.error
          ...
          "Rewriter.ProgUtils.pred_in_types: Expected pred definition"
              ...
              if AstDef.QualIdent.is_local qual_ident then return true
                ...
                | _ ->
                    Error.error (AstDef.Expr.to_loc expr)
                      "Rewriter.ProgUtils.is_expr_pure: Expected a function or \
                       a variable")
          | _ -> return true
        in
  ...
  let get_data_destrs_from_constr (qual_ident : qual_ident) : qual_ident list t
    ...
    | AstDef.Module.ConstrDef constr_def -> (
        let tp_name =
          ...
          | _ ->
              Error.error
                ...
                "Rewriter.ProgUtils.get_data_destrs_from_constr: Expected a \
                 variable"
        in

        let* symbol =
          find_and_reify (AstDef.QualIdent.to_loc tp_name) tp_name
        in
        match symbol with
        | AstDef.Module.TypeDef { type_def_expr = Some tp_expr; _ } -> (
            ...
            | App (Data (_, variant_decls), [], _) -> (
                ...
                | None ->
                    Error.error
                      (AstDef.QualIdent.to_loc qual_ident)
                      "Rewriter.ProgUtils.get_data_destrs_from_constr: \
                       Expected a variant declaration"
                | Some variant_decl ->
                    return
                      (Base.List.map variant_decl.variant_args
                         ~f:(fun var_decl ->
        ...
        | _ ->
            Error.error
              (AstDef.QualIdent.to_loc qual_ident)
              "Rewriter.ProgUtils.get_data_destrs_from_constr: Expected a type \
               definition")
    | _ ->
        Error.error
          (AstDef.QualIdent.to_loc qual_ident)
