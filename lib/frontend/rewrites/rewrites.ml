...
let rec rewrite_loops (stmt : Stmt.t) : Stmt.t Rewriter.t =
  ...
  | Loop loop ->
      ...
      let* ( loop_arg_var_decls,
        ...
        let curr_loop_args =
          Stmt.local_vars_accessed loop.loop_postbody |> Set.to_list
        in
      ...
      let* loop_body =
        let set_ret_vals_to_initial_args =
          List.map (Map.to_alist loop_ret_renaming_map)
            ~f:(fun (old_var, new_expr) ->
              Stmt.mk_assign ~loc:(Stmt.loc stmt) [ new_expr ]
        ...
        let recurse_call =
          ...
          Stmt.mk_call ~loc:(Stmt.loc stmt) ~lhs:lhs_list
        ...
        let cond_stmt =
          ...
          let then_stmt =
            Stmt.mk_block_stmt ~loc:(Stmt.loc stmt) [ loop_body; recurse_call ]
          in
      ...
      let postconds_exhale_stmts =
        if Callable.is_atomic callable_decl then
          let atomic_token_var =
            Expr.mk_var ~loc:(Stmt.loc stmt) ~typ:Type.atomic_token
              (QualIdent.from_ident
                 (Rewriter.ProgUtils.callable_au_token_ident
                    ~loc:(Stmt.loc stmt) callable_decl.call_decl_name))
          in
          ...
          [
            Stmt.mk_exhale_expr ~cmnt:(Some "au_return_stmt")
              ~loc:(Stmt.loc stmt)
              ~spec_error:
                (Stmt.mk_const_spec_error
                   "Could not prove atomic postcondition at return stmt.")
              (Expr.mk_app ~loc:(Stmt.loc stmt) ~typ:Type.perm
          ...
          List.map postconds_spec ~f:(fun spec ->
              ...
              Stmt.mk_exhale_expr ~loc:stmt.stmt_loc
                ...
                ~spec_error:
                  (Stmt.mk_const_spec_error
                     "Could not prove postconditions at return stmt.")
                expr)
      in
          ...
          | Fold ->
              ( Stmt.mk_inhale_expr ~loc:stmt.stmt_loc
                ...
                Stmt.mk_exhale_expr ~loc:stmt.stmt_loc
                  ...
                  ~spec_error:
                    (Stmt.mk_const_spec_error
                       "Could not prove predicate body for fold stmt.")
                  body_expr )
          | Unfold ->
              ( Stmt.mk_inhale_expr ~loc:stmt.stmt_loc
                ...
                Stmt.mk_exhale_expr ~loc:stmt.stmt_loc
                  ...
                  ~spec_error:
                    (Stmt.mk_const_spec_error
                       "Could not exhale predicate for unfold stmt.")
                  pred_expr )
          | _ -> assert false
        in
      ...
      | ProcDef _ ->
          ...
          let assert_stmt =
            Stmt.mk_assert_expr ~loc:stmt.stmt_loc
              ...
              ~spec_error:
                (Stmt.mk_const_spec_error
                   "Could not assert callable preconditions for call stmt.")
          ...
          let exhale_stmt =
            Stmt.mk_exhale_expr ~loc:stmt.stmt_loc
              ...
              ~spec_error:
                (Stmt.mk_const_spec_error
                   "Could not exhale callable preconditions for call stmt.")
              (Expr.mk_and
                 (List.map call_decl.call_decl_precond ~f:(fun spec ->
                      Expr.alpha_renaming spec.spec_form new_renaming_map)))
          in
      ...
      | Some body ->
          ...
          let* pre_conds, post_conds =
            if not (Callable.is_atomic c.call_decl) then
              ...
              let atomic_token_var =
                Expr.mk_var ~loc:(Stmt.loc body) ~typ:Type.atomic_token
                  (QualIdent.from_ident
                     (Rewriter.ProgUtils.callable_au_token_ident
                        ~loc:(Stmt.loc body) c.call_decl.call_decl_name))
              in
              ...
              let inhale_au =
                Stmt.mk_inhale_expr ~cmnt:(Some "au_precond")
                  ~loc:(Stmt.loc body)
                  (Expr.mk_app ~loc:(Stmt.loc body) ~typ:Type.perm
              ...
              let exhale_au =
                ...
                Stmt.mk_exhale_expr ~cmnt:(Some "au_postcond")
                  ~loc:(Stmt.loc body)
                  ~spec_error:
                    (Stmt.mk_const_spec_error
                       "Could not prove that atomic update has been committed.")
                  (Expr.mk_app ~loc:(Stmt.loc body) ~typ:Type.perm
          ...
          let new_body =
            Stmt.mk_block_stmt ~loc:(Stmt.loc body)
              (pre_conds @ [ body ] @ post_conds)
          in
          let new_proc =
            Callable.
              {
...
module AtomicityAnalysis = struct
  ...
  let take_atomic_step ~loc (state : atomicity_check) : atomicity_check =
    if List.is_empty state.au_opened && List.is_empty state.invs_opened then
      ...
      | true ->
          Error.error loc "Multiple atomic steps taken in a single atomic block"

  let take_non_atomic_step ~loc (state : atomicity_check) : atomicity_check =
    if List.is_empty state.au_opened && List.is_empty state.invs_opened then
      state
    else Error.error loc "Cannot take a non-atomic step inside an atomic block"
  ...
  let open_inv ~loc (inv_name, inv_args) atomicity_state : atomicity_check =
    if
      ...
      || not
           (Set.exists atomicity_state.mask ~f:(fun mask ->
                QualIdent.(mask = inv_name)))
    then Error.error loc "Invariant already opened"
    else
      {
    ...
    if
      not
        (List.exists atomicity_state.invs_opened ~f:(fun inv ->
             QualIdent.(inv.inv_name = inv_name)
             && List.for_all2_exn inv_args inv.inv_args ~f:Expr.alpha_equal))
    then Error.error loc "Invariant not already opened"
    else
      let invs_opened =
  ...
  let rewrite_au_cmnds ~(ghost_block : bool) (stmt : Stmt.t) :
    ...
    let rec rewrite_au_cmnds ~(ghost_block : bool) (stmt : Stmt.t) :
      ...
      | Basic (Call call_desc) ->
          ...
          then
            Error.error stmt.stmt_loc
              "Cannot call callable; invariant mask not satisfied"
          else
            ...
            if is_call_lhs_ghost then Rewriter.return stmt
            else if ghost_block then
              Error.error stmt.stmt_loc
      ...
      | Basic (AUAction auaction_desc) -> (
          ...
          | BindAU qual_iden ->
              ...
              let assign_stmt =
                Stmt.mk_assign ~loc:(Stmt.loc stmt)
          ...
          | OpenAU (token, callable, bound_vars) -> (
              ...
              | None ->
                  ...
                  let new_stmt =
                    Stmt.mk_block_stmt ~loc:(Stmt.loc stmt)
                      (exhale_stmt :: inhale_stmts)
                  in
                          ...
                          if not spec.spec_atomic then None
                            ...
                            Some
                              (Stmt.mk_exhale_expr
                                 ...
                                 ~spec_error:
                                   (Stmt.mk_const_spec_error
                                      "Could not prove preconditions for \
                                       abortAU.")
                                 (Expr.alpha_renaming spec.spec_form
                                    alpha_renaming_map)))
                    in
                          ...
                          if not spec.spec_atomic then None
                            ...
                            Some
                              (Stmt.mk_exhale_expr
                                 ...
                                 ~spec_error:
                                   (Stmt.mk_const_spec_error
                                      "Could not prove postconditions for \
                                       commitAU.")
                                 (Expr.alpha_renaming spec.spec_form
                                    alpha_renaming_map)))
                    in
...
let rec rewrite_ssa_stmts (s : Stmt.t) :
  ...
  | Basic basic_stmt -> (
      ...
      | Assign assign_stmt ->
          ...
          Rewriter.return
            Stmt.
              { s with stmt_desc = Basic (Assign { assign_lhs; assign_rhs }) }
      | Havoc qual_iden ->
