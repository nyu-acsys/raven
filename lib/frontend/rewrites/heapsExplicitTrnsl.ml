    ...
    Stmt.mk_assert_expr ~loc
      ~spec_error:
        (Stmt.mk_const_spec_error
           "Could not prove injectivity of iterated separating conjunction.")
      assert_expr
  in
  ...
  let* env_local_var_decls =
    let symbols =
      List.fold (inv_expr :: conds)
        ...
        ~f:(fun symbols cond -> Expr.symbols cond)
    in
  ...
  let postcond =
    let spec_form =
      let var_decls_forall =
        List.map universal_quants.univ_vars ~f:(fun (var, var_decl) -> var_decl)
      in
...
let generate_skolem_function (universal_quants : universal_quants)
    (var_decl : var_decl) ?(postconds : expr list = [])
    ?(optn_args : (var_decl * expr) list = []) ~loc : expr Rewriter.t =
  let open Rewriter.Syntax in
  let univ_quants_list = universal_quants.univ_vars in

  let skolem_fn_ident =
    Ident.fresh loc ("$skolem_" ^ Ident.to_string var_decl.var_name)
  in
...
let introduce_heaps_in_stmts ~loc ~fields_list ~preds_list ~au_preds_list body :
  ...
  let* field_heap_var_defs =
    Rewriter.List.map fields_list ~f:(fun field_name ->
        ...
        let* field_utils_id =
          Rewriter.ProgUtils.get_field_utils_id (Stmt.loc body) field_name
        in
        ...
        Rewriter.return
          ( { Stmt.var_decl = heap_var_decl; var_init = None },
            ...
            Stmt.mk_assume_expr ~loc:(Stmt.loc body) assume_expr2 ))
  in
  ...
  let* pred_heap_var_defs =
    Rewriter.List.map preds_list ~f:(fun pred_name ->
        ...
        let* pred_utils_id =
          Rewriter.ProgUtils.get_pred_utils_id (Stmt.loc body) pred_name
        in
  ...
  let* au_heap_var_defs =
    Rewriter.List.map au_preds_list ~f:(fun call_name ->
        ...
        let* au_utils_id =
          Rewriter.ProgUtils.get_au_utils_id (Stmt.loc body) call_name
        in
...
let rec rewrite_fpu (stmt : Stmt.t) : Stmt.t Rewriter.t =
  ...
  | Basic (Fpu fpu_desc) ->
      let* field_symbol =
        let* symbol =
          Rewriter.find_and_reify (Stmt.loc stmt) fpu_desc.fpu_field
        in
        match symbol with
      ...
      let* old_val =
        ...
        | None ->
            let* field_heap_symbol =
              let* symbol =
                Rewriter.find_and_reify (Stmt.loc stmt)
      ...
      let assert_stmt =
        Stmt.mk_assert_expr ~loc:stmt.stmt_loc
          ...
          ~spec_error:
            (Stmt.mk_const_spec_error "Could not prove validity of fpu.")
      ...
      let exhale_stmt =
        Stmt.mk_exhale_expr ~loc:stmt.stmt_loc
          ~cmnt:(Some ("FPU stmt: " ^ Stmt.to_string stmt))
          ~spec_error:
            (Stmt.mk_const_spec_error "Could not exhale fpu pre-value.")
    ...
    | Binder (Exists, var_decls, trgs, e, e_attr) ->
        let* subst =
          Rewriter.List.fold_left var_decls ~init:subst ~f:(fun map var_decl ->
              let* (new_expr : expr) =
                generate_skolem_function universal_quants var_decl
              in
              Rewriter.return
                (Map.add_exn map
          ...
          | App (Var qual_ident, args, _) -> (
              ...
              | CallDef c
                when Poly.(
                  ...
                  let assume_stmt =
                    ...
                    Stmt.mk_assume_expr ~loc
                      ...
                      (Expr.mk_binder
                         ...
                         (Expr.mk_binder ~loc ~typ:Type.bool Forall
                            univ_vars_list
                            (Expr.mk_app ~loc ~typ:Type.bool Expr.Ite
                               [
                                 (* m1(a,b,c) && l == f1(a, b, c) *)
                  ...
                  let stmts_list =
                    stmts_list
                    @ [ havoc_stmt; assume_stmt; eq_stmt; assume_heap_valid ]
                  in
      ...
      | Binder (Exists, var_decls, trgs, e, expr_attr) ->
          ...
          List.iter (Map.to_alist witnesses) ~f:(fun (id, witns) ->
              List.iter witns ~f:(fun (conds, e) ->
                  Logs.debug (fun m ->
                      m "id: %a, conds: %a, e: %a" Ident.pr id
                        (Fmt.Dump.list Expr.pr) conds (Fmt.Dump.option Expr.pr)
                        e)));

          let* skolemized_exprs =
            Rewriter.List.map var_decls ~f:(fun var_decl ->
                let* postconds, optn_args =
                  match Map.find witnesses var_decl.var_name with
                  | None -> Rewriter.return ([], [])
                  | Some witness_list ->
                      let witness_list =
                        List.filter witness_list ~f:(fun (conds, e) ->
                            Option.is_some e)
                      in
                      ...
                      let conds =
                        List.concat_map witness_list ~f:(fun (conds, _) ->
                            conds)
                      in
                      ...
                      let locals =
                        Set.filter symbols ~f:(fun s ->
                            QualIdent.is_local s
                            && not
                                 (List.exists univ_vars.univ_vars
                                    ~f:(fun (i, _) ->
                                      Ident.(i = QualIdent.unqualify s))))
                      in
                      ...
                      let* locals_var_decls =
                        Rewriter.List.map locals ~f:(fun qual_ident ->
                            let+ symbol =
                              Rewriter.find_and_reify var_decl.var_loc
                                qual_ident
                            in
                            match symbol with
                            | VarDef v -> v.var_decl
                      ...
                      Logs.debug (fun m ->
                          m
                            "Rewrites.HeapsExplicitTrnsl.WitnessComputation.elim_a1: \
                             locals_var_decls: %a"
                            (Fmt.Dump.list QualIdent.pr)
                            locals);

                      let optn_args =
                        optn_args
                        @ List.map locals_var_decls ~f:(fun var_decl ->
                              (var_decl, Expr.from_var_decl var_decl))
                      in
                ...
                let+ expr =
                  generate_skolem_function univ_vars var_decl ~postconds
          ...
          | CallDef c when Poly.(c.call_decl.call_decl_kind = Pred) ->
              ...
              let* witness_map =
                Rewriter.List.fold_left out_args_concrete_exprs
                  ~init:witness_map ~f:(fun map (out_arg, concrete_expr) ->
                    let* witnesses =
                    ...
                    let witness_map =
                      List.fold exist_vars ~init:witness_map
                        ~f:(fun witness_map var_decl ->
                          let existing_val =
                            Map.find_exn witness_map var_decl.var_name
                          in
    ...
    | App (Own, [ e1; e2; e3 ], _) ->
        ...
        *)
        ...
        let assert_heap_valid =
          Stmt.mk_assert_expr ~loc
            ~spec_error:
              (Option.value spec_error
                 ~default:(Stmt.mk_const_spec_error "Could not exhale stmt."))
            (Expr.mk_app ~loc ~typ:Type.bool (Expr.Var field_heap_valid_fn)
               [ field_heap_expr ])
        in
        ...
        let stmts_list =
          stmts_list @ [ havoc_stmt; assume_stmt; eq_stmt; assert_heap_valid ]
        in
          ...
          Stmt.mk_assert_expr ~loc
            ~spec_error:
              (Option.value spec_error
                 ~default:
                   (Stmt.mk_const_spec_error
                      "Could not exhale atomic update predicate."))
            (Expr.mk_app ~loc ~typ:Type.bool (Expr.Var au_heap_valid_fn)
               [ au_heap_expr ])
        in
          ...
          let assert_stmt =
            Stmt.mk_assert_expr ~loc
              ...
              ~spec_error:
                (Option.value spec_error
                   ~default:
                     (Stmt.mk_const_spec_error "Could not exhale pure_stmt."))
              assert_expr
          in
          ...
          | App (Var qual_ident, args, _) -> (
              ...
              | CallDef c
                when Poly.(
                  ...
                  let stmts_list =
                    stmts_list
                    @ [ havoc_stmt; assume_stmt; eq_stmt; assert_heap_valid ]
                  in
          ...
          | Assert ->
              ...
              if is_e_pure then
                ...
                let nondet_var =
                  Type.
                    {
                      var_name = Ident.fresh s.stmt_loc "$nondet";
                      var_loc = s.stmt_loc;
                      var_type = Type.bool;
                      var_const = true;
                      var_ghost = false;
                    ...
                    ?cmnt:
                      (Some
                         (Option.value ~default:(Stmt.to_string s)
                            spec.spec_comment))
                    ~loc:s.stmt_loc spec.spec_form
                in
                let assume_false_stmt =
                  Stmt.mk_assume_expr ~loc:s.stmt_loc (Expr.mk_bool false)
                in
                  ...
                  Stmt.Cond
                    {
                      ...
                      cond_else = Stmt.mk_block_stmt ~loc:s.stmt_loc [];
                    }
                in
          ...
          if QualIdent.(orig_ra_name = Predefs.lib_frac_mod_qual_ident) then
            ...
            let assert_stmt =
              Stmt.mk_assert_expr ~loc:s.stmt_loc
                ~spec_error:
                  (Stmt.mk_const_spec_error
                     "Could not assert sufficient permissions for field-read.")
                assert_expr
            in
          ...
          if QualIdent.(orig_ra_name = Predefs.lib_frac_mod_qual_ident) then
            ...
            let assert_stmt =
              Stmt.mk_assert_expr ~loc:s.stmt_loc
                ~spec_error:
                  (Stmt.mk_const_spec_error
                     "Could not assert sufficient permissions for field_write.")
                assert_expr
            in
            let assign_stmt =
