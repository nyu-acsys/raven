...
let tuple_arg_mismatch_error loc expected =
  Error.type_error loc
    (Printf.sprintf "Expected tuple with %s components."
...
module ProcessTypeExpr = struct
  let rec process_type_expr (tp_expr : type_expr) : type_expr Rewriter.t =
    ...
    | App (Data _, _tp_list, _tp_attr) ->
        ...
        Error.internal_error (Type.loc tp_expr)
  ...
  let rec expand_type_expr (tp_expr : type_expr) : Type.t Rewriter.t =
    ...
    | App (constr, tp_expr_list, tp_attr) -> (
        ...
        | Var qual_iden, [] -> (
            ...
            in
            let* qual_ident_def =
              Rewriter.Symbol.reify_type_def (Type.loc tp_expr) symbol
            in
...
let check_and_set (expr : expr) (given_typ_lb : type_expr)
  ...
  let+ given_typ_lb =
    ...
    with Msg (lbl, _loc, msg) -> Error.fail ?lbl (Expr.to_loc expr) msg
...
let rec process_expr (expr : expr) (expected_typ : type_expr) : expr Rewriter.t
  ...
  | App (constr, expr_list, expr_attr) -> (
      ...
      | Var qual_ident, args_list -> (
          ...
          let _ =
            Logs.debug (fun m ->
                m !"process_expr: ident: %{QualIdent}" qual_ident)
          in
  ...
  let process_stmt ?(new_scope = true) (expected_return_type : Type.t)
    ...
    let rec process_stmt ?(new_scope = true) stmt disam_tbl =
      ...
      let+ stmt_desc, disam_tbl =
        ...
        | Basic basic_stmt -> (
            ...
            | VarDef var_def ->
                ...
                let+ stmt, disam_tbl' =
                  ...
                  | Some expr ->
                      ...
                      let assign_desc =
                        Stmt.{ assign_lhs = [ var_expr ]; assign_rhs = expr }
                      in
            ...
            | Assign assign_desc -> (
                List.iter assign_desc.assign_lhs ~f:(fun expr ->
                    ...
                    | _ ->
                        Error.type_error stmt.stmt_loc
                          "Expected variables or var.field on left-hand side \
                           of assignment");
                ...
                | true -> (
                    ...
                    | App (Var proc_qual_ident, args, expr_attr) -> (
                        ...
                        if Predefs.is_qual_ident_au_cmnd proc_qual_ident then
                          ...
                          let* assign_lhs =
                            Rewriter.List.map assign_desc.assign_lhs
                              ~f:(fun expr ->
                                disambiguate_process_expr expr Type.any
                                  disam_tbl)
                          in
                          ...
                          let+ call_expr =
                            ...
                            disambiguate_process_expr expr expected_return_type
                              disam_tbl
                            (* TODO: <- replace this with the expected type *)
                          in
                ...
                | false -> (
                    ...
                    | _ ->
                        let assign_desc = Stmt.{ assign_lhs; assign_rhs } in
        ...
        | Cond cond_desc ->
            let* cond_test =
              disambiguate_process_expr cond_desc.cond_test Type.bool disam_tbl
            in
...
module ProcessModule = struct
  ...
  let check_implements_symbol interface_ident (symbol : Symbol.t)
    ...
    | VarDef var_def, VarDef orig_var_def -> (
        if var_def.var_decl.var_const && not orig_var_def.var_decl.var_const
        then
          Error.type_error loc
            (Printf.sprintf
               !"Cannot redeclare variable %{Ident} from interface \
                 %{QualIdent} as value"
               ident interface_ident)
        else if
          (not var_def.var_decl.var_const) && orig_var_def.var_decl.var_const
        then
          Error.type_error loc
            (Printf.sprintf
               !"Cannot redeclare value %{Ident} from interface %{QualIdent} \
                 as variable"
               ident interface_ident)
        else if
          var_def.var_decl.var_ghost && not orig_var_def.var_decl.var_ghost
        then
          Error.type_error loc
            (Printf.sprintf
        ...
        then
          ...
          Type.(var_def.var_decl.var_type <> orig_var_def.var_decl.var_type)
        then
          Error.type_error loc
            (Printf.sprintf
               !"%s %{Ident} must have type %{Type} according to interface \
                 %{QualIdent}"
               (Symbol.kind symbol |> String.uppercase)
  ...
  let rec process_module (m : Module.t) : Module.t Rewriter.t =
    ...
    let defined_symbols =
      List.fold mod_def
        ...
        ~f:(fun ids -> function
          | SymbolDef symbol -> Set.add ids (Symbol.to_name symbol) | _ -> ids)
    in
    ...
    let* ( mod_decl_returns,
           mod_decl_interfaces,
           interface_ident,
           (inherited_symbols, symbols_to_check) ) =
      let+ interface_opt =
      ...
      | Some (qual_interface_ident, interface_ident, ModDef interface) ->
          ( Some qual_interface_ident,
            ...
            interface_ident,
            List.fold interface.mod_def
              ~init:([], Map.empty (module Ident))
              ~f:(fun (inherited, to_check) -> function
                | Module.SymbolDef symbol ->
                    let ident = Symbol.to_name symbol in
                    if Set.mem defined_symbols ident then
                      (inherited, Map.add_exn to_check ~key:ident ~data:symbol)
                    else
                      let symbol =
                        match symbol with
                        | CallDef call_def ->
                            Module.CallDef (Callable.make_free call_def)
                        | _ -> symbol
                      in
    ...
    let+ mod_def =
      Rewriter.List.map (inherited_symbols @ m.mod_def) ~f:process_instr
    in
