...
module SymbolTbl = struct
  ...
  let rec map_to_string t =
    match t with
    | [] -> " "
    | (k, v) :: ms ->
        (QualIdent.to_string k ^ " -> " ^ QualIdent.to_string v ^ "\n")
        ^ map_to_string ms
  ...
  let rec to_string tbl =
    ...
    | t :: ts ->
        ...
        ^ " :: [ "
        ^ map_to_string (Map.to_alist (snd t))
        ^ " ]\n" ^ to_string ts
  ...
  let rec fully_qualified (id : qual_ident) tbl =
    match tbl with
    | [] -> id
    | (label, _) :: ts -> (
        match label with
  ...
  let rec add_helper tbl name fq_id =
    ...
    | t :: ts ->
        let label, map = t in
        let t' =
          try (label, Map.add_exn map ~key:name ~data:fq_id)
          with _ ->
            raise
              (Failure
                 ("duplicate exists: " ^ QualIdent.to_string name ^ " to "
                ^ QualIdent.to_string fq_id))
        in
        let ts' =
          match label with
          | None -> ts
  ...
  let add tbl name id =
    print_debug
      ("ADDING " ^ QualIdent.to_string name ^ " -> " ^ QualIdent.to_string id
     ^ "\n" ^ to_string tbl);
...
let option_disambiguate fn arg tbl =
  match arg with
  | None -> (None, tbl)
  | Some s ->
...
let rec list_disambiguate fn arg tbl =
  match arg with
  | [] -> ([], tbl)
  | l :: ls ->
...
module IdentDisambiguate = struct
  ...
  let new_ident_disambiguate iden (tbl : SymbolTbl.t) =
    print_debug ("New_Ident: " ^ Ident.to_string iden ^ "\n");
    let iden' = Ident.fresh iden.ident_name in
    let tbl =
      SymbolTbl.add tbl (QualIdent.from_ident iden) (QualIdent.from_ident iden')
    in
    (iden', tbl)
end
...
module QualIdentDisambiguate = struct
  let rec qual_ident_disambiguate (qual_iden : qual_ident) tbl =
    ...
    | None ->
        raise (Failure ("Variable Not Ref: " ^ QualIdent.to_string qual_iden))
    | Some id -> (id, tbl)
end
...
module TypeDisambiguate = struct
  ...
  and variant_decl_disambiguate (variant_decl : Type.variant_decl) tbl =
    let variant_name', tbl =
      new_ident_disambiguate variant_decl.variant_name tbl
    in
    (*  : ident; *)
    let variant_loc' = variant_decl.variant_loc in
    (*  : location; *)
    let variant_args', tbl =
      list_disambiguate var_decl_disambiguate variant_decl.variant_args tbl
    in
  ...
  and constr_disambiguate (tc : Type.constr) tbl =
    match tc with
    | Int | Bool | Unit | Ref | Perm | Bot | Any | Set | AtomicToken | Map ->
        (tc, tbl)
    | Var qual_ident ->
    ...
    | Data variant_decl_list ->
      (* TODO: Is this the right thing to do? 
        ...
        in
        (Data variant_decl_list', tbl)

  and type_disambiguate exp tbl =
    match exp with
    | App (tc, tp_list, tp_attr) ->
        ...
        let tp_attr', tbl = type_attr_disambiguate tp_attr tbl in

        (App (tc', tp_list', tp_attr'), tbl)
...
module ExprDisambiguate = struct
  let rec constr_disambiguate (const : Expr.constr) tbl :
    ...
    match const with
    | Null | Unit | Bool _ | Int _ | Empty | Not | Uminus | Eq | Gt | Lt | Geq
    | Leq | Diff | Union | Inter | Elem | Subseteq | And | Or | Impl | Plus
    | Minus | Mult | Div | Mod | Dot | Call | Read
    (* Ternary operators *)
    | Ite | Write | Own
    ...
    | Var qual_ident ->
        ...
        (Var qual_ident', tbl)
    | New tp_expr ->
        let tp_expr', tbl = type_expr_disambiguate tp_expr tbl in
        (New tp_expr', tbl)
  ...
  and expr_disambiguate (exp : expr) tbl : expr * SymbolTbl.t =
    match exp with
    | App (Read, exp_list, exp_attr) ->
        let exp_list', tbl =
          match exp_list with
          | [ h; t ] ->
              let t', tbl = expr_disambiguate t tbl in
    ...
    | Binder (bind, var_decl_list, exp, exp_attr) ->
        ...
        let var_decl_list', tbl =
          list_disambiguate var_decl_disambiguate var_decl_list tbl
        in
...
module StmtDisambiguate = struct
  let rec spec_disambiguate (spec : Stmt.spec) tbl =
    (* TODO: REVISIT THIS *)
    let spec_form', tbl = expr_disambiguate spec.spec_form tbl in
    (* : expr *)
  ...
  and call_desc_disambiguate (call_desc : Stmt.call_desc) tbl =
    let call_lhs', tbl =
      list_disambiguate qual_ident_disambiguate call_desc.call_lhs tbl
    in
    (*  : qual_ident list; *)
    let call_name', tbl = qual_ident_disambiguate call_desc.call_name tbl in
    (*  : qual_ident; *)
    let call_args', tbl =
      list_disambiguate expr_disambiguate call_desc.call_args tbl
    in

    (*  : expr list; *)
    let (call_desc' : Stmt.call_desc) =
      { call_lhs = call_lhs'; call_name = call_name'; call_args = call_args' }
    in

    (call_desc', tbl)

  and fold_desc_disambiguate (fold_desc : Stmt.fold_desc) tbl =
    let fold_expr', tbl = expr_disambiguate fold_desc.fold_expr tbl in
  ...
  and basic_stmt_desc_disambiguate (basic_stmt : Stmt.basic_stmt_desc) tbl :
      Stmt.basic_stmt_desc * (SymbolTbl.t * var_decl list) =
    match basic_stmt with
    | VarDef var_def ->
        let var_def', tbl = var_def_disambiguate var_def tbl in
        (VarDef var_def', (tbl, [ var_def'.var_decl ]))
    | Assume spec ->
        let spec', tbl = spec_disambiguate spec tbl in
        (Assume spec', (tbl, []))
    | Assert spec ->
        let spec', tbl = spec_disambiguate spec tbl in
        (Assert spec', (tbl, []))
    | New new_desc ->
        let new_desc', tbl = new_desc_disambiguate new_desc tbl in
        (New new_desc', (tbl, []))
    | Havoc expr_list ->
        let expr_list', tbl =
          list_disambiguate expr_disambiguate expr_list tbl
        in
        (Havoc expr_list', (tbl, []))
    | Call call_desc ->
        let call_desc', tbl = call_desc_disambiguate call_desc tbl in
    ...
    | Fold fold_desc ->
        let fold_desc', tbl = fold_desc_disambiguate fold_desc tbl in
        (Fold fold_desc', (tbl, []))
    | Unfold unfold_desc ->
        let unfold_desc', tbl = unfold_desc_disambiguate unfold_desc tbl in
  ...
  and loop_desc_disambiguate (loop_desc : Stmt.loop_desc)
    ...
    let loop_contract', tbl =
      list_disambiguate spec_disambiguate loop_desc.loop_contract tbl
    in
    (* : spec list; *)
    let loop_prebody', (tbl, locals) =
      stmt_disambiguate loop_desc.loop_prebody (tbl, locals)
    in
    (* : t; *)
    let loop_test', tbl = expr_disambiguate loop_desc.loop_test tbl in
    (* : expr; *)
    let loop_postbody', (tbl, locals) =
      stmt_disambiguate loop_desc.loop_postbody (tbl, locals)
    in
    (* : t; *)
    ...
    (loop_desc', (tbl, locals))

  and cond_desc_disambiguate (cond_desc : Stmt.cond_desc) (tbl, locals) =
    let cond_test', tbl = expr_disambiguate cond_desc.cond_test tbl in
    (* : expr; *)
    let tbl = SymbolTbl.push tbl in
    let cond_then', (tbl, locals) =
      stmt_disambiguate cond_desc.cond_then (tbl, locals)
    in
    (* : t; *)
    let tbl = SymbolTbl.pop tbl in
    let tbl = SymbolTbl.push tbl in
    let cond_else', (tbl, locals) =
      stmt_disambiguate cond_desc.cond_else (tbl, locals)
    in
    (* : t; *)
    let tbl = SymbolTbl.pop tbl in
    ...
    (cond_desc', (tbl, locals))

  and ghost_desc_disambiguate (ghost_desc : Stmt.ghost_desc) (tbl, locals) =
    let tbl = SymbolTbl.push tbl in
    let ghost_body', (tbl, locals) =
      list_disambiguate stmt_disambiguate ghost_desc.ghost_body (tbl, locals)
    in
    (* : t list; *)
    let tbl = SymbolTbl.pop tbl in
    ...
    (ghost_desc', (tbl, locals))

  and stmt_desc_disambiguate stmt_desc (tbl, locals) =
    match stmt_desc with
    | Block stmt_list ->
        let tbl = SymbolTbl.push tbl in
        let stmt_list', (tbl, locals) =
          list_disambiguate stmt_disambiguate stmt_list (tbl, locals)
        in
        let tbl = SymbolTbl.pop tbl in
        (Block stmt_list', (tbl, locals))
    | Basic basic_stmt_desc ->
        let basic_stmt_desc', (tbl, locals') =
          basic_stmt_desc_disambiguate basic_stmt_desc tbl
        in
        (Basic basic_stmt_desc', (tbl, List.append locals locals'))
    | Loop loop_desc ->
        let loop_desc', (tbl, locals) =
          loop_desc_disambiguate loop_desc (tbl, locals)
        in
        (Loop loop_desc', (tbl, locals))
    | Cond cond_desc ->
        let cond_desc', (tbl, locals) =
          cond_desc_disambiguate cond_desc (tbl, locals)
        in
        (Cond cond_desc', (tbl, locals))
    | Ghost ghost_desc ->
        let ghost_desc', (tbl, locals) =
          ghost_desc_disambiguate ghost_desc (tbl, locals)
        in
        (Ghost ghost_desc', (tbl, locals))
end
...
module CallableDisambiguate = struct
  let rec locals_to_string (m : (ident * Type.var_decl) list) =
    ...
    | l :: ls ->
        ...
        ^ " -> "
        ^ Ident.to_string (snd l).var_name
        ^ ",   " ^ locals_to_string ls
  ...
  let rec call_decl_disambiguate (call_decl : Callable.call_decl) tbl =
    ...
    in
    let call_decl_formals', tbl =
      list_disambiguate old_ident_disambiguate call_decl.call_decl_formals tbl
    in
    let call_decl_returns', tbl =
      list_disambiguate old_ident_disambiguate call_decl.call_decl_returns tbl
    in
    let call_decl_precond', tbl =
      list_disambiguate StmtDisambiguate.spec_disambiguate
        call_decl.call_decl_precond tbl
    in
    let call_decl_postcond', tbl =
      list_disambiguate StmtDisambiguate.spec_disambiguate
        call_decl.call_decl_postcond tbl
    in
  ...
  and proc_def_disambiguate (proc_def : Callable.proc_def) tbl =
    let rec map_append m (l : var_decl list) =
      match l with
      | [] -> m
    ...
    let proc_body', (tbl, locals) =
      option_disambiguate stmt_disambiguate proc_def.proc_body (tbl, [])
    in
    ...
    let proc_decl'' =
      {
        proc_decl' with
        call_decl_locals = map_append proc_decl'.call_decl_locals locals;
      }
    in
  ...
  and callable_disambiguate (call : Callable.t) tbl : Callable.t * SymbolTbl.t =
    match call with
    | FuncDef func_def ->
        let func_def', tbl = func_def_disambiguate func_def tbl in
        (FuncDef func_def', tbl)
    | ProcDef proc_def ->
        let proc_def', tbl = proc_def_disambiguate proc_def tbl in
...
module ModuleDisambiguate = struct
  let rec type_alias_disambiguate (type_alias : Module.type_alias) tbl =
    let type_alias_name', tbl =
      new_ident_disambiguate type_alias.type_alias_name tbl
    in
    let type_alias_def', tbl =
      option_disambiguate type_expr_disambiguate type_alias.type_alias_def tbl
    in
  ...
  and mod_alias_disambiguate (mod_alias : Module.module_alias) tbl =
    let mod_alias_name', tbl =
      new_ident_disambiguate mod_alias.mod_alias_name tbl
    in
    let mod_alias_type', tbl =
      type_expr_disambiguate mod_alias.mod_alias_type tbl
    in
    let mod_alias_def', tbl =
      option_disambiguate type_expr_disambiguate mod_alias.mod_alias_def tbl
    in
  ...
  and import_directive_disambiguate (imp_dir : Module.import_directive) tbl :
    ...
    match imp_dir with
    | ModImport tp_exp ->
        let tp_exp', tbl = type_expr_disambiguate tp_exp tbl in
        (ModImport tp_exp', tbl)
    | MemImport qual_id ->
        let qual_id', tbl = qual_ident_disambiguate qual_id tbl in
        (MemImport qual_id', tbl)

  and member_def_disambiguate (mem_def : Module.member_def) tbl :
      Module.member_def * SymbolTbl.t =
    match mem_def with
    | TypeAlias tp_alias ->
        let tp_alias', tbl = type_alias_disambiguate tp_alias tbl in
    ...
    | Import imp_dir ->
        ...
        (Import imp_dir', tbl)
    | ModDef mod_def ->
        let mod_def', tbl = mod_def_disambiguate mod_def tbl in
        (ModDef mod_def', tbl)
    | FieldDef field_def ->
        let field_def', tbl = field_def_disambiguate field_def tbl in
        (FieldDef field_def', tbl)
    | ValDef var_defn ->
        let var_defn', tbl =
          StmtDisambiguate.var_def_disambiguate var_defn tbl
        in
        (ValDef var_defn', tbl)
    | CallDef call ->
        let call', tbl = callable_disambiguate call tbl in
        (CallDef call', tbl)
  ...
  and mod_def_disambiguate mod_def tbl =
    match mod_def with
    | ModImpl mod1 ->
        let mod', tbl = module_disambiguate mod1 tbl in
        (ModImpl mod', tbl)
    | ModAlias mod_alias ->
        let mod_alias', tbl = mod_alias_disambiguate mod_alias tbl in
  ...
  and module_disambiguate mod1 tbl =
    let rec extract_members (mod_defs_list : Module.member_def list)
      ...
      | def :: defs -> (
          ...
          | FieldDef field_def ->
              let fields =
                Map.add_exn fields ~key:field_def.field_name
                  ~data:field_def.field_type
              in
              extract_members defs
                (rep, mod_defs, mod_aliases, fields, types, callables, vars)
          | ValDef v ->
    ...
    let mod_def', tbl =
      list_disambiguate member_def_disambiguate mod1.mod_def tbl
    in
    ...
    let rep, mod_defs, mod_aliases, fields, types, callables, vars =
    ...
    let tbl =
      ...
      | t :: ts -> (
          ...
          | Some r ->
              print_debug
                ("REP TYPE Adding: "
                ^ Ident.to_string mod_decl_name'
                ^ " -> " ^ Ident.to_string r);
              t
              :: SymbolTbl.add ts
                   (QualIdent.from_ident mod_decl_name')
                   (QualIdent.make [ mod_decl_name' ] r))
    in
