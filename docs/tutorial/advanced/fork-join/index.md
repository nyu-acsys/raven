...
`join` needs an explicit call to `token_unique`, right where {{ref sec:idea-a-token}} said the proof needs it — after
...
(sitting inside a `{! ... !}` ghost block — {{ref sec:ghost-blocks-erasure}}'s
syntax — since its condition depends on a ghost variable, and an ordinary `if` in a `proc` can't
branch on ghost state; the same `{! !}` syntax you'll see again in Appendix A if you go looking
for more resource-algebra examples.) Every one of these
