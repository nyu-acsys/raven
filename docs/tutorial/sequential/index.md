...
those start in [Part 2](../ownership/). If you've used an *auto-active* verification tool
...
actually gets it checked instead ({{ref sec:control-flow}} below covers this properly; the short version is that it's
...
3. **[`hot_day.rav`](./exercises/hot_day.rav)** — the busiest day in a per-day hit log,
   deliberately designed to reward the `forall`-over-`exists` preference from {{ref sec:quantifiers}} above when
   proving rather than assuming a quantified fact.

## What's next

[Part 2](../ownership/) puts this same counter on the heap — one `field`, one `Ref` — and
