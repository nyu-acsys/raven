...
`decreases` clause (the same discipline, and the same good-practice advice, as Part 1
§4). Second, and more fundamentally, a `lemma` can never affect the *running* program: it can
...
`decreases` clause you've written so far actually resolves to), `Library.LexOrder[A, B]` (Part
1 §5's lexicographic combinator, generic over any two well-founded orders `A` and `B`),
...
proof (an explicit `lemma`, as in §4).
