...
let parse_and_check_cu ?(tbl = SymbolTbl.create ()) smtEnv top_level_md_ident
    file_name front_end_out_chan =
  ...
  let tbl, processed_md =
    Rewrites.process_module (*Rewrites.Rewriting.process_module*)
      ~tbl processed_md
  in
...
let parse_and_check_all file_names =
  ...
  let lib_file =
    Stdlib.Filename.dirname (Sys.get_argv ()).(0)
    ^ "/../lib/library/resource_algebra.rav"
  in
  let tbl = SymbolTbl.create () in
  let smtEnv, tbl =
    parse_and_check_cu ~tbl smtEnv Predefs.lib_ident lib_file front_end_out_chan
  in
